...
# TODO make short options instead of positional arguments
if [ $# -gt 3 ]; then
  printf 1>&2 "%s\n" "Usage: $0 [METADIR] [CONFIG] [PROPS]

  Reads a stream of datum_uuid,datum pairs from stdin
  looks up PROPS in METADIR (all properties in \$PWD/metadir by default)
  outputs biorg
...
build_prop () {
    ...
    local propdir=$(jq -r ".$prop.dir" "$config")
    ...
    if [ "$prop_uuid" ]; then
        ...
        local prop_label=$(jq -r ".$prop.label" "$config")
        if [ ! "$prop_label" ]; then
...
build_node () {
    ...
    local root_uuid=$(grep -o "$stdin_uuid" "$metadir/props/$root/index.csv")
    if [ ! "$root_uuid" ]; then
        return
    fi
    ...
    while (( ${#props[@]} != 0 )); do
        ...
        local parent=$(jq -r ".$prop.parent" "$config")
...
test_build-biorg_succeeds() {
    code
    assert "bash $script $metadir $config \"hostname hostdate guestname guestdate filepath filesize\""
}
...
value1
EOF
    assert_equals "$(mock)" "$(bash $script $metadir $config "hostname hostdate guestname guestdate filepath filesize")"
}
