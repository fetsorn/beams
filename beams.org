...
function write_prop(prop, prop_dir, parent, parent_uuid) {
...
function write_node(temp, datum) {
    ...
    write_prop("datum", "datum", "", "")

    write_prop("guestname", "name", config["parent"]["guestname"], uuids[config["parent"]["guestname"]])

    write_prop("guestdate", "date", config["parent"]["guestdate"], uuids[config["parent"]["guestdate"]])

    write_prop("hostname", "name", config["parent"]["hostname"], uuids[config["parent"]["hostname"]])

    write_prop("hostdate", "date", config["parent"]["hostdate"], uuids[config["parent"]["hostdate"]])

    write_prop("privacy", "privacy", config["parent"]["privacy"], uuids[config["parent"]["privacy"]])

    write_prop("tag", "tag", config["parent"]["tag"], uuids[config["parent"]["tag"]])

    write_prop("filepath", "filepath", config["parent"]["filepath"], uuids[config["parent"]["filepath"]])

    # TODO replace with (config["parent"][prop] != "")
    if (values["filepath"] != "") {

        write_prop("filesize", "filesize", config["parent"]["filesize"], uuids[config["parent"]["filesize"]])

        write_prop("filetype", "filetype", config["parent"]["filetype"], uuids[config["parent"]["filetype"]])

        write_prop("moddate", "date", config["parent"]["moddate"], uuids[config["parent"]["moddate"]])

        write_prop("filehash", "", config["parent"]["filehash"], uuids[config["parent"]["filehash"]])
    }
}
BEGIN {
    temp_cmd = "mktemp -d"
...
/^:/ {
    ...
    if (property_block==1) {
        ...
        gsub("^[ \t]+", "", prop_value);
        gsub("[ \t]+$", "", prop_value);
        values[config["name"][prop_label]]=prop_value;
        next;
    }
}
{
