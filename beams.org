...
update_file () {
    ...
    printf 1>&2 "update: %s\n" "$csvpath"
    if [ -f "$new/$csvpath" ]; then
        changed_uuids="cat \"$new/$csvpath\" | grep \"$uuid\" | cut -c 1-64"
        # use grep because ripgrep bugs out on an empty pattern file
        # https://github.com/BurntSushi/ripgrep/issues/1332
        unchanged_lines="cat \"$main/$csvpath\" | grep -vf <(${changed_uuids})"
        # remove empty newline in case unchanged_lines is empty
        cat <(eval "$unchanged_lines") <(cat "$new/$csvpath" | grep "$uuid") | sed '/^$/d' > "$main/$csvpath"
    else
        printf 1>&2 "skip %s\n" "$csvpath"
    fi
}
...
# the rest is content-addressable so can be appended as is and sorted later
append_file "$new" "$main" "$uuid" "props/date/index.csv"
append_file "$new" "$main" "$uuid" "props/name/index.csv"
append_file "$new" "$main" "$uuid" "props/filepath/index.csv"
append_file "$new" "$main" "$uuid" "props/filesize/index.csv"
append_file "$new" "$main" "$uuid" "props/filetype/index.csv"
append_file "$new" "$main" "$uuid" "props/filepath/index.csv"
append_file "$new" "$main" "$uuid" "pairs/privacy/index.csv"
#+end_src
