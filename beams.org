...
cat "$filename" | sort | uniq | grep -o "^$uuid,.*" | sed "s/^$uuid,//" | head -n 1
#+end_src
** unescape-json.awk
#+begin_src awk :tangle scripts/unescape-json.awk

# unescape-json.awk
#
# unescapes some characters when converting from json
...
build_biorg () {
    ...
    datum=$(printf "%s" "$datum_escaped" | awk -f scripts/unescape-json.awk)
    ...
    filepath=$(printf "%s" "$filepath_escaped" | awk -f scripts/unescape-json.awk)
    ...
    filetype=$(printf "%s" "$filetype_escaped" | awk -f scripts/unescape-json.awk)
...
parallel -a "$temp" -j 4 build_biorg {} "{#}"
printf 1>&2 "\n"
#+end_src
...
build_json () {
    ...
    event=$(jq -c --arg UUID "$datum_uuid" \
                  --arg FILE_PATH "$filepath" \
                  --arg GUEST_NAME "fetsorn" \
                  --arg GUEST_DATE "$gustdate" \
                  --arg HOST_NAME "fetsorn" \
                  --arg HOST_DATE "$hostdate" \
                  --arg DATUM "$datum" \
                  '{$UUID,$FILE_PATH,$GUEST_NAME,$GUEST_DATE,$HOST_NAME,$HOST_DATE,$DATUM}' <(printf "[]"))
    printf "%s\n" "$event"
...
function write_node(temp, datum_uuid, filepath, filesize, filetype, moddate, filehash, guestname, guestdate, hostname, hostdate, privacy, datum) {
...
BEGIN {
    ...
    mkdir_cmd = "mkdir -p " temp "/props/filepath " temp "/props/filesize " temp "/props/filetype " temp "/props/date " temp "/props/name " temp "/props/privacy " temp "/props/datum " temp "/pairs";
    system(mkdir_cmd);
    counter=0;
}
# heading line
/^\*/ {
    # write previous node
    if (node==1) {
        write_node(temp, datum_uuid, filepath, filesize, filetype, moddate, filehash, guestname, guestdate, hostname, hostdate, privacy, datum);
    }
...
_gc () {
    ...
    cat "$csvfile" | rg -f "$rgfile" | sort | uniq | sort -t "," -k 2 | sponge "$csvfile"
}
...
_gc "$metadir/props/datum/index.csv" <(printf ".*")

# remove dead datum pairs
datum_props=$(cat "$metadir/props/datum/index.csv" | cut -c 1-64)
_gc "$metadir/pairs/datum-guestdate.csv" <(printf "%s" "$datum_props")
_gc "$metadir/pairs/datum-hostdate.csv"  <(printf "%s" "$datum_props")
_gc "$metadir/pairs/datum-hostname.csv"  <(printf "%s" "$datum_props")
_gc "$metadir/pairs/datum-guestname.csv" <(printf "%s" "$datum_props")
_gc "$metadir/pairs/datum-filepath.csv"  <(printf "%s" "$datum_props")
_gc "$metadir/pairs/datum-privacy.csv"   <(printf "%s" "$datum_props")

# remove garbage filepath props
filepath_pairs=$(cat "$metadir/pairs/datum-filepath.csv" | cut -c 66- )
_gc "$metadir/props/filepath/index.csv" <(printf "%s" "$filepath_pairs")

# remove dead filepath pairs
filepath_props=$(cat "$metadir/props/filepath/index.csv" | cut -c 1-64)
_gc "$metadir/pairs/filepath-moddate.csv"  <(printf "%s" "$filepath_props")
_gc "$metadir/pairs/filepath-filesize.csv" <(printf "%s" "$filepath_props")
_gc "$metadir/pairs/filepath-filetype.csv" <(printf "%s" "$filepath_props")
_gc "$metadir/pairs/filepath-filehash.csv" <(printf "%s" "$filepath_props")

# remove garbage filesize props
filesize_pairs=$(cat "$metadir/pairs/filepath-filesize.csv" | cut -c 66-)
_gc "$metadir/props/filesize/index.csv" <(printf "%s" "$filesize_pairs")

# remove garbage filetype props
filetype_pairs=$(cat "$metadir/pairs/filepath-filetype.csv" | cut -c 66-)
_gc "$metadir/props/filetype/index.csv" <(printf "%s" "$filetype_pairs")
...
# the rest is content-addressable so can be appended as is and sorted later
append_file "$new" "$main" "props/date/index.csv"
append_file "$new" "$main" "props/name/index.csv"
append_file "$new" "$main" "props/filepath/index.csv"
append_file "$new" "$main" "props/filesize/index.csv"
append_file "$new" "$main" "props/filetype/index.csv"
append_file "$new" "$main" "props/filepath/index.csv"
append_file "$new" "$main" "pairs/privacy/index.csv"

# deduplicate, sort and remove orphans
bash scripts/gc "$main"
#+end_src
