...
if [ $# -gt 1 ]; then
  ...
  looks up the rest of metadata in METADIR (\$PWD/metadir by default)
  outputs biorg

  EXAMPLE:
    cat metadir/props/datum/index.csv | $0 metadir > ../ops/index.bi.org"
  exit 3
fi
...
build_biorg () {
    ...
    # skip if requested uuid is not in metadir
    datum_uuid=$(grep -o "$stdin_uuid" "$metadir/props/datum/index.csv")
    if [ ! "$datum_uuid" ]; then
        return
    fi
    ...
    if [ -f "$metadir/pairs/datum-hostname.csv" ]; then
        hostname_uuid=$(bash "$DIR/lookup" "$metadir/pairs/datum-hostname.csv" "$datum_uuid")
        if [ "$hostname_uuid" ]; then
            hostname=$(bash "$DIR/lookup" "$metadir/props/name/index.csv" "$hostname_uuid")
            printf ":HOST_NAME: %s\n" "$hostname"
        fi
    fi

    if [ -f "$metadir/pairs/datum-hostdate.csv" ]; then
        hostdate_uuid=$(bash "$DIR/lookup" "$metadir/pairs/datum-hostdate.csv" "$datum_uuid")
        if [ "$hostdate_uuid" ]; then
            hostdate=$(bash "$DIR/lookup" "$metadir/props/date/index.csv" "$hostdate_uuid")
            printf ":HOST_DATE: <%s>\n" "$hostdate"
        fi
    fi

    if [ -f "$metadir/pairs/datum-guestname.csv" ]; then
        guestname_uuid=$(bash "$DIR/lookup" "$metadir/pairs/datum-guestname.csv" "$datum_uuid")
        if [ "$guestname_uuid" ]; then
            guestname=$(bash "$DIR/lookup" "$metadir/props/name/index.csv" "$guestname_uuid")
            printf ":GUEST_NAME: %s\n" "$guestname"
        fi
    fi

    if [ -f "$metadir/pairs/datum-guestdate.csv" ]; then
        guestdate_uuid=$(bash "$DIR/lookup" "$metadir/pairs/datum-guestdate.csv" "$datum_uuid")
        if [ "$guestdate_uuid" ]; then
            guestdate=$(bash "$DIR/lookup" "$metadir/props/date/index.csv" "$guestdate_uuid")
            printf ":GUEST_DATE: <%s>\n" "$guestdate"
        fi
    fi

    if [ -f "$metadir/pairs/datum-tag.csv" ]; then
        tag_uuid=$(bash "$DIR/lookup" "$metadir/pairs/datum-tag.csv" "$datum_uuid")
        if [ "$tag_uuid" ]; then
            tag=$(bash "$DIR/lookup" "$metadir/props/tag/index.csv" "$tag_uuid")
            printf ":TAG: %s\n" "$tag"
        fi
    fi

    if [ -f "$metadir/pairs/datum-filepath.csv" ]; then
        filepath_uuid=$(bash "$DIR/lookup" "$metadir/pairs/datum-filepath.csv" "$datum_uuid")
        if [ "$filepath_uuid" ]; then
            filepath_escaped=$(bash "$DIR/lookup" "$metadir/props/filepath/index.csv" "$filepath_uuid")
            filepath=$(printf "%s" "$filepath_escaped" | bash "$DIR/unescape")
            printf ":FILE_PATH: %s\n" "$filepath"

            filesize_uuid=$(bash "$DIR/lookup" "$metadir/pairs/filepath-filesize.csv" "$filepath_uuid")
            if [ "$filesize_uuid" ]; then
                filesize=$(bash "$DIR/lookup" "$metadir/props/filesize/index.csv" "$filesize_uuid")
                printf ":FILE_SIZE: %s\n" "$filesize"
            fi

            filetype_uuid=$(bash "$DIR/lookup" "$metadir/pairs/filepath-filetype.csv" "$filepath_uuid")
            if [ "$filetype_uuid" ]; then
                filetype_escaped=$(bash "$DIR/lookup" "$metadir/props/filetype/index.csv" "$filetype_uuid")
                filetype=$(printf "%s" "$filetype_escaped" | bash "$DIR/unescape")
                printf ":FILE_TYPE: %s\n" "$filetype"
            fi

            moddate_uuid=$(bash "$DIR/lookup" "$metadir/pairs/filepath-moddate.csv" "$filepath_uuid")
            if [ "$moddate_uuid" ]; then
                moddate=$(bash "$DIR/lookup" "$metadir/props/date/index.csv" "$moddate_uuid")
                printf ":MOD_DATE: <%s>\n" "$moddate"
            fi

            filehash=$(bash "$DIR/lookup" "$metadir/pairs/filepath-filehash.csv" "$filepath_uuid")
            if [ "$filehash" ]; then
                printf ":FILE_HASH: %s\n" "$filehash"
            fi
        fi
    ...
    datum_escaped=$(bash "$DIR/lookup" "$metadir/props/datum/index.csv" "$datum_uuid")
    datum=$(printf "%s" "$datum_escaped" | bash "$DIR/unescape")
    printf "%s\n" "$datum"
}
...
# if there are datum uuids on stdin, get them
if [[ ! -t 0 ]]; then
    while read -r line; do
        build_biorg "$line" "$counter";
        counter=$(($counter + 1));
...
# otherwise get all datums
    cat "$metadir/props/datum/index.csv" | cut -c 1-64 | \
    while read -r line; do
        build_biorg "$line" "$counter";
        counter=$(($counter + 1));
    done
...
value1
EOF
    assert_equals "$(mock)" "$(bash $script $metadir)"
}
