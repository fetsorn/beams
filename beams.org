...
build_biorg () {
    ...
    tag_uuid=$(bash scripts/lookup "metadir/pairs/datum-tag.csv" "$datum_uuid")
    if [ "$tag_uuid" ]; then
        tag=$(bash scripts/lookup "metadir/props/tag/index.csv" "$tag_uuid")
        printf ":TAG: %s\n" "$tag"
    fi
...
ln -sf "$bk" sync-bk
# rebuild biorg
cat "$main/props/datum/index.csv" | bash scripts/build-biorg > "$inbox"
#+end_src
* tests
