...
if [ $# -gt 2 ]; then
  printf 1>&2 "%s\n" "Usage: $0 [METADIR] [PROPS]

  Reads a stream of datum_uuid,datum pairs from stdin
  looks up PROPS in METADIR (all props \$PWD/metadir by default)
  outputs biorg
...
# TODO move config to json and jq
declare -A config_unescape config_date config_propdir config_parent config_label

if [ "$2" ]; then
    config_props="$2"
else
    config_props="hostdate guestdate hostname guestname filepath filesize filetype moddate tag"
fi

config_root="datum"

config_unescape["filetype"]=1
config_unescape["datum"]=1

config_date["hostdate"]=1
config_date["guestdate"]=1
config_date["moddate"]=1

config_propdir["hostdate"]="date"
config_propdir["guestdate"]="date"
config_propdir["moddate"]="date"

config_propdir["hostname"]="name"
config_propdir["guestname"]="name"

config_parent["hostdate"]="datum"
config_parent["hostname"]="datum"
config_parent["guestdate"]="datum"
config_parent["guestname"]="datum"
config_parent["tag"]="datum"
config_parent["filepath"]="datum"
config_parent["moddate"]="filepath"
config_parent["filetype"]="filepath"
config_parent["filesize"]="filepath"

config_label["hostdate"]="HOST_DATE"
config_label["hostname"]="HOST_NAME"
config_label["guestdate"]="GUEST_DATE"
config_label["guestname"]="GUEST_NAME"
config_label["tag"]="TAG"
config_label["filepath"]="FILE_PATH"
config_label["moddate"]="MOD_DATE"
config_label["filetype"]="FILE_TYPE"
config_label["filesize"]="FILE_SIZE"

# tbn PARENT_UUID PROP
# tbn "12401595" "hostname"
# prints data
# sets two variables, ${PROP} and ${PROP}_uuid
tbn () {
    local parent_uuid="$1"
    local prop="$2"
    local parent=${config_parent[$prop]}

    # variables to set on return
    declare -n VALUE="$prop"
    declare -n UUID="${prop}_uuid"

    local pair="$metadir/pairs/$parent-$prop.csv"

    # override common prop dirs like name/ and date/
    local propdir=${config_propdir[$prop]}
    if [ ! "$propdir" ]; then
        propdir="$prop"
    fi
    ...
    # if prop is root, ignore pairs and print as datum
    if [ "$config_root" == "$prop" ]; then
        prop_uuid="$parent_uuid"
        prop_value=$(bash "$DIR/lookup" "$metadir/props/$propdir/index.csv" "$prop_uuid")

        if [ ${config_unescape[$prop]} ]; then
            prop_value=$(printf "%s" "$prop_value" | bash "$DIR/unescape")
        fi
    ...
    if [ -f "$pair" ]; then
        prop_uuid=$(bash "$DIR/lookup" "$pair" "$parent_uuid")
        if [ "$prop_uuid" ]; then
            prop_value=$(bash "$DIR/lookup" "$metadir/props/$propdir/index.csv" "$prop_uuid")
            ...
            local prop_label=${config_label[$prop]}
            if [ ! "$prop_label" ]; then
                prop_label="$prop"
            fi

            if [ ${config_date[$prop]} ]; then
...
build_biorg () {
    ...
    root_uuid=$(grep -o "$stdin_uuid" "$metadir/props/$config_root/index.csv")
    if [ ! "$root_uuid" ]; then
        return
    fi
    ...
    while (( ${#props[@]} != 0 )); do
        prop=${props[-1]}
        parent=${config_parent[$prop]}
        if [ ! "$parent" ]; then parent="$prop"; fi
        if [ "$parent" == "$config_root" ]; then
            tbn "$root_uuid" "$prop"
            processed["$prop"]=1
        elif [ ${processed[$parent]} ]; then
            parent_var="${parent}_uuid"
            tbn "${!parent_var}" "$prop"
            processed["$prop"]=1
        else
            props=( "$prop" "${props[@]}" )
...
# otherwise get all datums
    while read -r line; do
        build_biorg "$line" "$counter";
        counter=$((counter + 1));
    done < cat "$metadir/props/datum/index.csv" | cut -c 1-64
fi
