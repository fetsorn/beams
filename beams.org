...
# tbn ROOT ROOT_UUID PROP
# tbn "datum" "12401595" "hostname"
# prints data
...
tbn () {
    ...
    local pair="$metadir/pairs/$root-$prop.csv"

    # override common prop dirs like name/ and date/
    # TODO get override switch and propdir value from config
    local propdir=""
    if [ "$prop" == "hostdate" ] || [ "$prop" == "guestdate" ] || [ "$prop" == "moddate" ]; then
        propdir="date"
    elif [ "$prop" == "hostname" ] || [ "$prop" == "guestname" ]; then
        propdir="name"
    else
        propdir="$prop"
    fi
    ...
    # treat root prop differently
    # TODO get and root status, unescape and print switches from config
    if [ "$prop" == "datum" ]; then
        prop_uuid="$root_uuid"
    ...
    if [ -f "$pair" ]; then
        local prop_uuid=$(bash "$DIR/lookup" "$pair" "$root_uuid")
        if [ "$prop_uuid" ]; then
            prop_value=$(bash "$DIR/lookup" "$metadir/props/$propdir/index.csv" "$prop_uuid")
            # optionally unescape
            # TODO get unescape switch from config
            if [ "$prop" == "filetype" ]; then
                prop_value=$(printf "%s" "$prop_value" | bash "$DIR/unescape")
            fi
            # optionally wrap dates in brackets
            # TODO get wrap switch from config
            VALUE="$prop_value"
            UUID="$prop_uuid"
            if [ "$prop" == "hostdate" ] || [ "$prop" == "guestdate" ] || [ "$prop" == "moddate" ]; then
                printf ":$prop: <%s>\n" "$prop_value"
...
build_biorg () {
    ...
    # TODO get root prop and dir from config
    ...
    # TODO get props from config
    tbn "datum" "$datum_uuid" "hostname"

    tbn "datum" "$datum_uuid" "hostdate"

    tbn "datum" "$datum_uuid" "guestname"

    tbn "datum" "$datum_uuid" "guestdate"

    tbn "datum" "$datum_uuid" "tag"

    tbn "datum" "$datum_uuid" "filepath"
    if [ "$filepath" ]; then
        tbn "filepath" "$filepath_uuid" "filesize"

        tbn "filepath" "$filepath_uuid" "filetype"

        tbn "filepath" "$filepath_uuid" "moddate"
    fi

    printf ":END:\n"

    tbn "datum" "$datum_uuid" "datum"
}
